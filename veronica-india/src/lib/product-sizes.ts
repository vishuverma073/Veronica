...
type SkuLike = { dimensionValues?: Record<string, string> };
type DimensionLike = {
  name: string;
  values: { value: string; sortOrder: number }[];
};

/** Unique size values from SKUs — used by MSW list projection. */
...
/** Merge unique sizes from a product list in stable first-seen order. */
export function collectAvailableSizes(products: { sizes: string[] }[]): string[] {
  const order = new Map<string, number>();
  let idx = 0;
  for (const product of products) {
    for (const size of product.sizes) {
      if (!order.has(size)) order.set(size, idx++);
    }
  }
